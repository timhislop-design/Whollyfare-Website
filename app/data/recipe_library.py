...
def recipes_for_sale_items(
    ...
    if proteins:
        ...
        if not matched_proteins:
            matched_proteins = {p.lower() for p in proteins}

    if not matched_proteins:
        matched_proteins = None  # no filter, let everything through
...
def get_recipe_shopping_items(recipe: dict) -> list[dict]:
    """
    Return only the non-pantry-stable ingredients for a recipe —
    the items that need to be purchased this week.
    """
    return [i for i in recipe["ingredients"] if not i["pantry_stable"]]


def pool_ingredients(recipes: list[dict]) -> list[dict]:
    """
    Deduplicate ingredients across a week's recipes.
    When the same ingredient appears in multiple meals, the quantity is
    the MAX across all uses (buy once, cover all meals) rather than the sum.

    Returns a flat list of {name, qty, unit, category, used_in_meals} dicts.
    POC simplification: matches on exact ingredient name.
    Phase 2: fuzzy match + unit conversion (e.g. "1.5 lbs" + "2 lbs" → "2 lbs").
    """
    pool: dict[str, dict] = {}
    for recipe in recipes:
        for ing in recipe["ingredients"]:
            key = ing["name"].lower()
            if key not in pool:
                pool[key] = {**ing, "used_in_meals": [recipe["name"]]}
            else:
                # Take the max quantity (covers all uses)
                if ing["qty"] > pool[key]["qty"]:
                    pool[key]["qty"] = ing["qty"]
                pool[key]["used_in_meals"].append(recipe["name"])

    return sorted(pool.values(), key=lambda x: x["category"])


def recipe_summary(recipe: dict) -> str:
    """One-line summary string for display."""
    flags = recipe["dietary_flags"]
    badges = []
    if flags.get("gluten_free"):  badges.append("GF")
    if flags.get("dairy_free"):   badges.append("DF")
    if flags.get("vegetarian"):   badges.append("VEG")
    if flags.get("vegan"):        badges.append("VEGAN")
    badge_str = f" [{', '.join(badges)}]" if badges else ""
    return (f"{recipe['name']}{badge_str} · {recipe['primary_protein']} · "
            f"{recipe['cuisine']} · {recipe['active_minutes']}min · "
            f"{recipe['complexity']}")
